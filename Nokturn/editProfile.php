	...
	if(!(isset($_SESSION['loggedIn']))) {
		...
		if($phone != $_SESSION['user_phone'] && $phone!="")
		{
			if((strlen($phone) < 9) || (strlen($phone) > 20)) {
				$all_ok = false;
				$_SESSION['e_phone'] = "Phone must have 2-20 characters!";
			}
		...
		{
			if((strlen($street) < 2) || (strlen($street) > 30)) {
				$all_ok = false;
				$_SESSION['e_street'] = "Street must have 2-30 characters!";
			}
		...
		{
			if((strlen($post_code) < 2) || (strlen($post_code) > 30)) {
				$all_ok = false;
				$_SESSION['e_post_code'] = "Post code must have 2-30 characters!";
			}
		}
		else
		...
		if($city != $_SESSION['user_city'] && $city!="")
		{
			if((strlen($city) < 2) || (strlen($city) > 100)) {
				$all_ok = false;
				$_SESSION['e_city'] = "City must have 2-100 characters!";
			}
		...
		if($country != $_SESSION['user_country'] && $country!="")
		{
			if((strlen($country) < 2) || (strlen($country) > 100)) {
				$all_ok = false;
				$_SESSION['e_country'] = "Country must have 2-100 characters!";
			}
		...
		if($name != $_SESSION['user_name'] && $name!="")
		{
			if((strlen($name) < 2) || (strlen($name) > 255)) {
				$all_ok = false;
				$_SESSION['e_name'] = "Name must have 2-255 characters!";
			}
		...
		if($surname != $_SESSION['user_surname'] && $surname!="")
		{
			if((strlen($surname) < 2) || (strlen($surname) > 255)) {
				$all_ok = false;
				$_SESSION['e_surname'] = "Surname must have 2-255 characters!";
			}
				...
				else 
				{
					...
					if($all_ok == true) {
							...
							post_code='$post_code', city='$city', country='$country' WHERE id=$_SESSION[user_id]")) 
						{
							...
							$_SESSION['user_post_code'] = $post_code;
							$_SESSION['user_city'] = $city;
							$_SESSION['user_country'] = $country;
							$_SESSION['user_password'] = $password;

							if($_SESSION['user_type'] == 0) header('Location: user.php');
							else if ($_SESSION['user_type'] == 1) header('Location: adminHomePage.php');
				...
				<div class="row">
					...
					<div class="col-xl-10 col-md-9 col-sm-10 col-xs-10">
						<div class="content-form">
							<p class="font-paragraph">Edit your data!</p>
							<p>Put changes into fields you want to be replaced</p>
							<form method="POST">
								...
								?>
									<input type="submit" value="Update" class="btn btn-primary btn-adjust btn-margin">
							</form>
							<?php
								if($_SESSION['user_type'] == 0) {
									echo '<a href="user.php">
										<div class="btn btn-primary btn-adjust btn-red">Cancel</div>
									</a>';
								}
